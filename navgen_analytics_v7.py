...
def main():
    ...
    alert_data = raw_alert_data.results.values.tolist()  # Assign raw alert JSON to variable
    df_alerts = pd.DataFrame(alert_data)  # Place alert data into a dataframe
    df_alerts_exploded = df_alerts.explode('ttps')  # Flatten out the 'ttps' object

    # Rearrange some columns
    column_to_move = df_alerts_exploded.pop("process_name")
    df_alerts_exploded.insert(0, "process_name", column_to_move)
    column_to_move = df_alerts_exploded.pop("process_sha256")
    df_alerts_exploded.insert(1, "process_sha256", column_to_move)
    column_to_move = df_alerts_exploded.pop("ttps")
    df_alerts_exploded.insert(2, "ttps", column_to_move)

    # Add a new column and strip off 'MITRE_' from ttp data so we have a column of *just* MITRE TTPs.
    df_alerts_exploded['mitre_technique'] = df_alerts_exploded['ttps'].str.extract(r'(?<=MITRE_)(.*?)(?=\_)')
    ...
    with open(f'{filetimestamp}_{args.project}_attack_cb_metadata_score_alert_count.json', 'w',
